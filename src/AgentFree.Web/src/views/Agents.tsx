...
import { Card, Typography, Button, Tag, Space, Modal, Form, Input, Select, Popconfirm, message, Drawer, Row, Col, Avatar, Tooltip } from 'antd'
import {
  PlusOutlined,
  EditOutlined,
  DeleteOutlined,
...
const agentTypeOptions = [
  { value: 'Goldfish', label: '🐠 AgentFree (自研)' },
...
export default function Agents() {
  ...
  const handleStatus = async (agent: Agent, status: string) => {
    try {
      if (status === 'Active') {
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'Active') {
      return <Tag icon={<CheckCircleOutlined />} color="success" style={{ borderRadius: 6 }}>运行中</Tag>
    }
    return <Tag icon={<CloseCircleOutlined />} style={{ borderRadius: 6 }}>已停止</Tag>
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <Text type="secondary">管理和配置所有智能体服务</Text>
        </div>
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={6} key={agent.id}>
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
                    <Tag color={style.tag} style={{ borderRadius: 6, margin: 0 }}>
                      {agent.agentType}
                    </Tag>
                    {agent.baseUrl && (
                      <Tooltip title={agent.baseUrl}>
                        <Tag icon={<GlobalOutlined />} style={{ borderRadius: 6, margin: 0, fontSize: 11, padding: '2px 6px' }}>{agent.baseUrl}</Tag>
                      </Tooltip>
                    )}
                    {agent.apiKey && (
                      <Tag icon={<KeyOutlined />} style={{ borderRadius: 6, margin: 0, fontSize: 11, padding: '2px 6px' }}>🔑</Tag>
                    )}
                  </div>
      ...
      >
        <Form form={form} layout="vertical" style={{ fontSize: 13 }}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="baseUrl" label="BaseURL" rules={[{ required: true, message: '请输入服务地址' }]}>
                <Input placeholder="http://127.0.0.1:5100" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="apiKey" label="KEY">
                <Input.Password placeholder="认证密钥" />
              </Form.Item>
            </Col>
          </Row>
          ...
          <Form.Item name="agentId" label="智能体ID">
            <Input placeholder="唯一标识（可选）" />
          </Form.Item>
          
          {/* 描述 */}
          <Form.Item name="description" label="描述">
            <Input.TextArea rows={2} placeholder="可选：描述这个智能体的功能" />
          </Form.Item>
        </Form>
      </Modal>
          ...
          <div>
            <Card size="small" style={{ marginBottom: 16, borderRadius: 12, border: '1px solid #f0f0f0' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                ...
                {detailAgent.baseUrl && <Tag icon={<GlobalOutlined />}>{detailAgent.baseUrl}</Tag>}
              </div>
            </Card>
